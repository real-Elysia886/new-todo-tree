...
QUnit.test('handles empty field value', function (assert) {
    const q = parse('tag:');
    assert.deepEqual(q.fields.tag, ['']);
});

QUnit.test('handles quoted field value', function (assert) {
    const q = parse('tag:"TODO FIXME"');
    assert.deepEqual(q.fields.tag, ['TODO FIXME']);
});
